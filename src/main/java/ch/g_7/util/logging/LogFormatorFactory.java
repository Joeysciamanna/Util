...
public final class LogFormatorFactory {
	...
	public static ILogFormator getDefault() {
		return new ILogFormator() {
			...
			public String format(LogMessage message) {
				...
				return  Formator.fillEnd("[" + message.getLevel() + "] ",' ', 10) + "["+ date +"]  " +
	...
	public static ILogFormator getOneLiner() {
		return new ILogFormator() {
			@Override
			public String format(LogMessage message) {
				return Formator.fillEnd("[" + message.getLevel() + "] ",' ', 10) + "["+ new SimpleDateFormat("dd-MM-yyy HH:mm").format(new Date()) +"]  " +
	...
	public static ILogFormator getTimeLose() {
		return new ILogFormator() {
			@Override
			public String format(LogMessage message) {
				return Formator.fillEnd("[" + message.getLevel() + "] ",' ', 10) + message.getSimpleMessage() + ":\n" + message.getStackTrace();
			}
		};
	}
